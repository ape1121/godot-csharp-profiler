...
public sealed class EndToEndTests
{
    ...
    public void Malformed_stale_duplicate_and_sequence_gap_preserve_completed_display()
    {
        ...
        var stale = StrictWireAdapter.Serialize(new BatchMessage(1, 0, Token, snapshot.Generation - 1,
        ...
        var gap = StrictWireAdapter.Serialize(new StateMessage(1, 0, Token, snapshot.Generation,
        ...
        Assert.False(loop.Editor.Receive(StrictWireAdapter.Serialize(new HelloMessage(1, 0, Token, "runtime", 100))));
    ...
    public void Incompatible_major_is_rejected_without_state_or_result_mutation()
    {
        ...
        {
            new("kind", new WireString("hello")), new("major", new WireInteger(2)),
    ...
    private static RuntimeSourceBatch Batch(CaptureSource source, long id, long value, long calls) =>
        new(source, source != CaptureSource.Sampling, false, new QualityCounters(1, 0, 0, 0), [new(id, value, calls)]);
    private static WireMap Configure(long generation) => StrictWireAdapter.Serialize(new ConfigureMessage(1, 0, Token,
    ...
    private static WireMap Stop(long generation, long sequence) => StrictWireAdapter.Serialize(new StopMessage(1, 0, Token, generation, sequence, Fingerprint));
